...
$(document).on('click', '.msg-import', function() {
	html = "";
	var memoNo = $(this).data('value');
	var statCd = $(this).data('stat-cd');
	...
	content.forEach((value, index, array) => {
		...
		if (path != 'detailSender') {
			html += `
					<div class="d-flex bg-light">
						<div class="msg-import" data-value=`+ value.memoNo + ` data-stat-cd=` + value.statCd + `>`
			//중요 쪽지인 경우와 아닌 경우		
			if (value.statCd == "MM005") {
		...
		html += `
				<div class="w-25 col-4 d-flex position-relative">
					<a class="stretched-link inbox-link" href="/msg/`+ path + `?memoNo=` + value.memoNo + `&statCd=` + value.statCd + `">` + value.deptNm + ` ` + value.cstCdNm + ` ` + value.userNm + `</a>
				</div>
				<div class="col-8 bg-light d-flex">
					<div class="col-8">
						<a class="d-block inbox-link" href="/msg/`+ path + `?memoNo=` + value.memoNo + `&statCd=` + value.statCd + `">`
		if(value.mngCdNm1=='열람'){
				html += `<span>`+value.memoTtl+`</span>`;
			}else if(value.mngCdNm1=='미열람'){
				html += `<span style="color: black;">`+value.memoTtl+`</span>`;
			}else if(value.mngCdNm1=='중요'){
				html += `<span style="color: blue;">`+value.memoTtl+`</span>`;
			}else{
				html += `<span>`+value.memoTtl+`</span>`;
			}		
		html += `
