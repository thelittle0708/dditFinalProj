<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="kr.or.ddit.reservation.vo.OffmVO"%>
...
<div class="card mb-3">
  ...
  <div class="card-body position-relative">
    <div class="row">
      <div class="col-lg-12">
        <h3 id="roomTitle" >회의실 목록</h3>
			<div class="table-responsive scrollbar">
			  <table class="table">
			    <thead>
			      <tr>
			        <th scope="col">희의실명</th>
			        <th scope="col">회의실 보유자산</th>
			      </tr>
			    </thead>
			    <tbody>
				<c:forEach var="offmVO" items="${data}" varStatus="stat">
			      <tr>
			        <td class="offm-number" ><button class="btn btn-primary me-1 mb-1" type="button">${offmVO.offmNm}</button></td>
					<td><c:forEach var="offmFxtrVO" items="${offmVO.offmFxtrsVOList}" varStatus="innerStat">
						<span style="margin-right: 10px;">${offmFxtrVO.fxtrsNm}</span>
					</c:forEach>
					<input type="hidden" value="${offmVO.offmNo}"/>
					</td>
			      </tr>
				</c:forEach>
			    </tbody>
	...
	<div class="modal-content border">
	  <form id="addEventForm" autocomplete="off">
		<div class="modal-header px-x1 bg-light border-bottom-0">
		  <h5 class="modal-title" id="offmNm"></h5>
		...
		<input type="hidden" id="offmNo" value="">
		</div>
...
<script>
var offmNumberCells = document.querySelectorAll('.offm-number');

offmNumberCells.forEach(function(cell) {
	cell.addEventListener('click', function() {
		var offmNm = this.textContent;
		var offmNoInput = this.parentElement.querySelector('input[type="hidden"]');
		var offmNo = offmNoInput.value;
		document.getElementById('offmNm').textContent = offmNm;
		document.getElementById('offmNo').value = offmNo;
		  ...
		  // 부모 요소에 추가합니다.
		  const parentContainer = document.getElementById('roomNumber');
		...
		$(function(){
			var request = $.ajax({
				url : "/reservationOffm/calendar/"+offmNo, // 값 불러오기
				method : "POST",
				dataType : "JSON",
				traditional: true,
				async: false, //동기
...
// 				console.log(data); // log로 데이터 찍어주기
	...
	insertBtn.onclick = function(){
		...
		var offmNo = document.getElementById('offmNo').value;
		console.log("offmNo"+offmNo);
	    ...
	    var redi;
	 	...
	 	$.ajax({
			url : "/reservationOffm/insertForm",
