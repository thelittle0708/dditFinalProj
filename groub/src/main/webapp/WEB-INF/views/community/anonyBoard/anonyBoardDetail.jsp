				...
				<tbody>
					...
					<tr>
						<td style="text-align: right;">
							<fmt:formatDate value="${data.mdfcnDt}" pattern="yyyy-MM-dd HH:mm" />
						</td>
					</tr>
					<tr>
...
<!-- 			</h3> -->
<%-- 			<div align="right"  style="margin-left: 40px;"><fmt:formatDate value="${data.mdfcnDt}" pattern="yyyy-MM-dd HH:mm" /></div> --%>
...
<!-- 		</div> -->
		...
		<div class="d-flex justify-content-end mb-2 py-5 pt-3 pb-0">
			...
			<a href="/community/anonyboard/list" class="btn btn-secondary  me-1 mb-1 " type="button">목록</a>
		</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="scrollinglongcontentLabel">익명게시판</h5>
		...
		<div id="commentArea" class="row">
			<c:forEach var="co" items="${comment}" varStatus="stat">
				...
				<c:if test="${co.delDt==null}">
					...
					<div class="col-md-1 py-2 text-center" ><input class="fas fa-user-alt"> </div>
					<c:if test="${co.cmntLv==0}">
						<div class="col-md-8 py-2 " id="comment" data-cmntno="${co.cmntNo}" data-cmnt-cn="${co.cmntCn}"	style="padding-left: ${co.cmntLv * 20}px;">${co.cmntCn}</div>
					</c:if>
					<c:if test="${co.cmntLv!=0}">
						<div class="col-md-8 py-2" id="comment" data-cmntno="${co.cmntNo}" data-cmnt-cn="${co.cmntCn}" style="padding-left: ${co.cmntLv * 20}px;">↳ ${co.cmntCn}</div>
					</c:if>
				...
				<div class="modal fade" id="scrollinglongcontent1" data-keyboard="false" tabindex="-1" aria-labelledby="scrollinglongcontentLabel" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="scrollinglongcontentLabel">익명 게시판_댓글</h5>
								<button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
							</div>
							<div class="d-flex text-center">
						...
						<div class="modal-content position-relative">
							...
							<div class="modal-body p-0">
								<div class="rounded-top-3 py-3 ps-4 pe-6 bg-light">
									<h4 class="mb-1" id="modalExampleDemoLabel">익명 댓글 수정</h4>
								</div>
								<div class="p-4 pb-0">
									<form onsubmit="return false;">
										<div class="mb-3">
											<label class="col-form-label" for="recipient-name">내용을 입력해 주세요</label> <input class="form-control" id="writeModComment" type="text" />
										</div>
									</form>
								</div>
							</div>
				...
				<!-- 댓글 수정 모달 끝-->
				<!-- 대댓글 작성-->
				<div class=" col-md-12 py-2  commentInputArea" id="commentInputArea_${co.cmntNo}" style="display: none;">
					<div class="d-flex">
...
$("#edit").click(function() {
    // update 컨트롤러로 이동
    window.location.href = "/community/anonyboard/update?pstNo=" + ${data.pstNo};
});
...
$("#btnDeleteConfirm").click(function(){
	...
	if(btnDel=='true'){
		console.log("true!!!")
		$.ajax({
            url: "/community/anonyboard/deletePost",
...
$("#cmntBtn").click(function() {
	...
	//console.log(data);
	  ...
	  $.ajax({
		 url:"/community/anonyboard/commentRegister" ,
		 contentType:"application/json;charset:utf-8",
		 data:JSON.stringify(data),
		 type:"post",
		 dataType:"text",
...
$(document).on("click", ".commentDelBtn", function() {
    ...
    $('#commentDelBtnConfirm').click(function(){
        ...
        if(commentDelBtnConfirm=='true'){
            console.log("true!!!")
            $.ajax({
                url: "/community/anonyboard/deleteCommentPost",
                contentType: "application/json;charset:utf-8",
                data: JSON.stringify(data),
                type: "post",
                dataType: "text",
...
$(document).on("click", "#commentModModal", function() {
    ...
    $("#commentModModalConfirm").click(function(){
    	...
    	$.ajax({
    		url:"/community/anonyboard/commentModify",
    		contentType:"application/json;charset:utf-8",
    		data:JSON.stringify(data),
    		type:"post",
    		dataType:"text",
...
function rereply(obj){
	...
	$.ajax({
		url:"/community/anonyboard/commentInComment" ,
		 contentType:"application/json;charset:utf-8",
		 data:JSON.stringify(data),
		 type:"post",
		 dataType:"text",
