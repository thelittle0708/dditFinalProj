...
<c:set value="${eml}" var="eml" />

<div class="card mb-1 d-flex justify-content-center">
	...
	<div class="card-body d-flex justify-content-between">
		<div>
			<a class="btn btn-falcon-default btn-sm" href="/mail/inbox" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="뒤로가기" data-bs-original-title="뒤로가기"><svg class="svg-inline--fa fa-arrow-left fa-w-14" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="arrow-left" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" data-fa-i2svg="">
		...
		<div class="d-flex">
			<div class="dropdown font-sans-serif">
				<button id="mail-set" class="btn btn-falcon-default text-600 btn-sm dropdown-toggle dropdown-caret-none ms-2" type="button" id="email-settings" data-bs-toggle="dropdown"
				...
				<div class="dropdown-menu dropdown-menu-end border py-2" id="mail-setting"
		...
		<div class="row">
			<div class="col-md d-flex">
				<div class="avatar avatar-2xl">
					<img class="rounded-circle" src="${eml.userImg}" alt="">
				</div>
				<div class="flex-1 ms-2">
					<h5 class="mb-0">${eml.emlTtl}</h5>
					<a class="text-800 fs--1" href="#!">
					<c:if test="${eml.userNm!=null}">
						<span class="fw-semi-bold">${eml.deptNm}과 ${eml.cstCdNm} ${eml.userNm}</span>
					</c:if>
						<span class="ms-1 text-500">${eml.dsptchEml}</span>
					</a>
				</div>
			</div>
			<div class="col-md-auto ms-auto d-flex align-items-center ps-6 ps-md-3">
				<small>${eml.dsptchDt}</small>
			</div>
		</div>
	</div>
...
// 	var csrfToken = $("input[name='_csrf']").val();
	...
	$('#mail-answer').on('click',()=>{
		location.href="/mail/compose?dsptchEml=${eml.dsptchEml}"
	});
	
	$('#mail-delete').on('click',()=>{
		$.ajax({
			url:'/mail/delete',
			type:'post',
			data:{
	...
	$('#mail-set').on('click',()=>{
		...
		if(`${eml.emlStts}`=='MM003'||`${param.sort}`=='trash'){
			var html = "";
			html += `<a class="dropdown-item" href="#!" id="restore">복원하기</a>`;
			$('#mail-setting').html(html);
		}
